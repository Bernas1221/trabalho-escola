...
export default function App() {
  ...
  async function register(){
    ...
    try{
      ...
      setUser(username); setXp(0); setScreen("menu"); updateRanking();
    } catch(e){ console.error(e); setError("Erro ao criar usuário"); }
  }
  ...
  async function login(){
    ...
    try{
      ...
      if(data.password !== password){ setError("Senha inválida"); return; }
      setUser(username); setXp(data.score||0); setScreen("menu"); updateRanking();
    }catch(e){ console.error(e); setError("Erro no login"); }
  }
  ...
  async function handleCorrect(){
    ...
    await setDoc(doc(db,"users",user),{password, score:newXP});
    if(streak+1>=3){ setLevel(prev=>prev+1); setStreak(0); }
    setShowOverlay(true); setTimeout(()=>setShowOverlay(false),2000);
    next(); updateRanking();
  }
  ...
  async function handleWrong(){
    ...
    await setDoc(doc(db,"users",user),{password, score:newXP});
    if(lives-1<=0){ setScreen("menu"); setLives(3); updateRanking(); return; }
